...
impl Config {
    pub fn load() -> Result<Self, String> {
        let token_file = env::var("FFMPEG_AGENT_TOKEN_FILE")
            .map_err(|_| "FFMPEG_AGENT_TOKEN_FILE is required")?;
        let token = fs::read_to_string(token_file)
            .map_err(|_| "cannot read token file")?
            .trim()
            .as_bytes()
            .to_vec();
        if token.len() < 32 || token.len() > 1024 || token.iter().any(|c| !c.is_ascii_graphic()) {
        ...
        Ok(Self {
            ...
            job_timeout: Duration::from_secs(number(
                "FFMPEG_AGENT_JOB_TIMEOUT_SECS",
                3600,
                1,
                86400,
            )?),
